...
void USB_PMA2Buffer(uint8_t endpoint)
{
	...
	uint32_t *src = (uint32_t *) (PMAAddr + btable[USB_ADDRn_RX] * 2);
	uint16_t *dest = (uint16_t *) RxTxBuffer[endpoint].RXB;
...
void USB_Buffer2PMA(uint8_t endpoint)
{
	...
	uint16_t *src = RxTxBuffer[endpoint].TXB;
	uint32_t *dest = (uint32_t *) (PMAAddr + btable[USB_ADDRn_TX] * 2);
